...
import { formatDateKeyShort, isDateKey } from "@/lib/date-keys";
import { createTask, deleteTask, setTaskDone, setTaskProject } from "@/lib/actions/tasks";
          ...
          {tasks.map((task) => {
            ...
            return (
              <div key={task.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
                ...
                {isDateKey(task.scheduled_date) && (
                  <span className="text-ink-muted font-mono text-xs tabular-nums">
                    {formatDateKeyShort(task.scheduled_date)}
                  </span>
                )}
                ...
                <Select
                  ...
                  onChange={(event) =>
                    run(() => setTaskProject(task.id, event.target.value || null))
                  }
